...
function BhaktapurTable() {
  const [bookings, setBookings] = useState([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [filteredBookings, setFilteredBookings] = useState([]);
  const [location, setLocation] = useState('Bhaktapur'); // Set default to Bhaktapur
  const [BhaktapurMiddlemen, setBhaktapurMiddlemen] = useState([]);

  // Fetch middlemen from Bhaktapur
  useEffect(() => {
    const fetchBhaktapurMiddlemen = async () => {
      try {
        console.log('Starting to fetch Bhaktapur middlemen...');
        ...
        console.log('Bhaktapur middlemen IDs:', response.data.middlemen.map(m => m._id));
        setBhaktapurMiddlemen(response.data.middlemen);
      } catch (error) {
        console.error('❌ Error fetching Bhaktapur middlemen:', error.response ? error.response.data : error.message);
      }
    };
    fetchBhaktapurMiddlemen();
  }, []);

  // Fetch bookings and filter by Bhaktapur middlemen
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        ...
        // Filter bookings where the middleman is in Bhaktapur
        console.log('Filtering bookings with Bhaktapur middlemen IDs:', BhaktapurMiddlemen.map(m => m._id));
        const bookingsWithStatus = response.data
          .filter((booking) => {
            const isMatch = BhaktapurMiddlemen.some((middleman) => {
    ...
    if (BhaktapurMiddlemen.length > 0) {
      console.log('Bhaktapur middlemen available, fetching bookings...');
      fetchBookings();
    } else {
      console.log('No Bhaktapur middlemen available yet, skipping booking fetch.');
    }
  }, [BhaktapurMiddlemen]);

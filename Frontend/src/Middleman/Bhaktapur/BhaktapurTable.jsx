...
function BhaktapurTable() {
  ...
  // Fetch bookings for Bhaktapur location
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        setBookings(BhaktapurBookings);
        setFilteredBookings(BhaktapurBookings);
      } catch (error) {
  ...
  const handleStatusChange = (bookingId, newStatus) => {
    ...
    localStorage.setItem('bookingStatuses', JSON.stringify(savedStatuses));
  };
              ...
              {filteredBookings.length > 0 ? (
                ...
                <tr>
                  <td colSpan="9" className="py-4 text-center text-gray-500">
                    No bookings found for Bhaktapur location
                  </td>
                </tr>
              )}
            </tbody>

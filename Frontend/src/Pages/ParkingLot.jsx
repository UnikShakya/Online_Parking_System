...
const ParkingLot = () => {
    ...
    const handleSpotClick = (spotId) => {
        ...
        if (bookedSpots.includes(spotId)) {
            return; // Prevent selecting booked spots
        }
        ...
        return (
            <div key={rowId} className="flex justify-center mb-4">
                {Array.from({ length: numSpots }).map((_, index) => {
                    ...
                    const isBooked = bookedSpots.includes(spotId); // Check if spot is booked
                    return (
                        <div
                            key={spotId}
            ...
            <div className="flex w-full max-w-5xl">
                ...
                <div className="flex flex-col ml-4 w-20">
                    {Array.from({ length: 5 }).map((_, index) => (
                        <div
                            key={`side-${index}`}
                            className={`w-12 h-12 m-1 rounded border ${bookedSpots.includes(`side-${index}`) ? "bg-red-500" : "bg-gray-300 hover:bg-gray-400"} cursor-pointer`}
